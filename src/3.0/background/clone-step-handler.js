pygmy3_0.cloneStepHandler = (function() {
	...
	function handlePutResponse(status, response, sendResponse, stepId, deploymentProcessId)
	{
	...
	function handleGetResponse(response, sendResponse, stepId, deploymentProcessId, url)
	{
		...
		nanoajax.ajax({url: url, method: "PUT", body: JSON.stringify(response)}, 
			function (status, response) { handlePutResponse(status, response, sendResponse, stepId, deploymentProcessId); });
	}
	...
	function cloneStep(request, sender, sendResponse)
	{
		if (request.message == 'clone-step')
		{
			...
			getJsonResponse(url, function (response) { handleGetResponse(response, sendResponse, stepId, deploymentProcessId, url); } );
